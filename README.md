...
**Biblioteca COTECNOVA** 
Sistema de gestión de préstamos que permite manejar el catálogo de libros de la biblioteca: registrar libros, asociarlos a una editorial y a un género, y controlar cuántos ejemplares hay disponibles (stock). Es el prototipo de software del proyecto integrador **"Sistema de Gestión de Préstamos para la Biblioteca COTECNOVA"**, de Tecnología en Sistemas de Información.
...
La Biblioteca COTECNOVA de Cartago, Valle del Cauca, presta material bibliográfico a estudiantes y docentes de los programas presenciales y a distancia de la institución. Su acervo es de **7.869 libros**, y el servicio lo atiende un equipo pequeño: en el estudio participan 2 bibliotecarios, y esa es una de las razones por las que la clasificamos como empresa **pequeña**. Su operación gira alrededor de un solo servicio: préstamo y devolución de material bibliográfico.
...
| 5 | Crecimiento de la demanda | La institución crece y abre nuevos programas, presenciales y a distancia, lo que aumenta el uso de la biblioteca y deja el servicio actual corto. |
...
En este primer corte el catálogo (libros, editoriales y géneros) está implementado. El flujo de préstamos está modelado en el MER y se usa con datos CSV simulados para las estadísticas del dashboard, pero todavía no está migrado a la base de datos.
...
- **Escala del inventario:** 7.869 libros en el acervo, de los cuales unos 5.313 están procesados en el aplicativo institucional y unos 1.334 están en el Excel paralelo.
...
- **Usuarios y roles:** los usuarios se identifican con tipo de identificación (CC, TI, etc.) y número, y se autentican con correo verificado. Hoy solo existe el rol de administrador (bibliotecario), que ve el panel de administración con la lista de libros, usuarios con correo verificado, préstamos y multas. La ficha exige dos roles diferenciados, **usuario** y **administrador**, así que el rol de usuario final queda pendiente.
...
|---|---|---|---|
| Usuario | `users` | Persona que se autentica en el sistema (hoy el bibliotecario; más adelante también el usuario final) | Implementada |
...
| Libro-Género | `libro_genero` | Relación muchos a muchos entre libros y géneros | Modelada (ver la nota de la sección 5.3) |
...
**Entidad pendiente de modelar:** la ficha incluye reservas en línea, pero el MER actual no tiene una tabla de reservas. A futuro se piensa agregarla (por ejemplo, `reserva` con usuario, libro, fecha y estado) cuando se construya ese módulo.

## 4. Diccionario de Datos

**Tablas implementadas en la base de datos (con migraciones):** `users`, `editorials`, `generos`, `libros`.
**Tablas del modelo completo, aún sin migrar:** `autor`, `ejemplar`, `ubicacion`, `prestamo` y las tres tablas de relación.
...
<img width="1109" height="1175" alt="image" src="https://github.com/user-attachments/assets/adbc3c22-f9ba-41cb-b01f-98cb770d6b12" />
...
|---|---|---|
| Editorial → Libro | 1:N | Una editorial publica muchos libros; cada libro es de una sola editorial |
| Autor ↔ Libro (por `Autor_Libro`) | N:M | Un autor escribe varios libros y un libro puede tener varios autores |
| Libro ↔ Género (por `Libro_Genero`) | N:M | Un libro puede tener varios géneros y un género agrupa muchos libros |
| Libro → Ejemplar | 1:N | Un libro tiene una o varias copias físicas |
| Ubicación → Ejemplar | 1:N | En una ubicación se guardan muchos ejemplares |
| Usuario → Préstamo | 1:N | Un usuario puede hacer muchos préstamos |
| Préstamo ↔ Ejemplar (por `Prestamo_Ejemplar`) | N:M | Un préstamo puede incluir varios ejemplares y un ejemplar se presta muchas veces a lo largo del tiempo |

### 5.2 Relaciones implementadas en este corte

- `Editorial` 1:N `Libro`: una editorial publica muchos libros; cada libro pertenece a una editorial (`libros.editorial_id`).
- `Genero` 1:N `Libro`: un género agrupa muchos libros; cada libro pertenece a un género (`libros.genero_id`).
- `User` consulta y gestiona el catálogo (autenticación para administrar libros, editoriales y géneros).

### 5.3 Diferencias entre el MER completo y lo implementado

| Aspecto | MER completo | Implementado en las migraciones | Por qué |
|---|---|---|---|
| Género del libro | N:M por `Libro_Genero` | 1:N: `libros.genero_id` (un género principal por libro) | Simplificación para el primer corte; se cambia a tabla puente cuando se migre el resto del modelo |
| Usuario | `Usuario` con nombres, apellidos, teléfono, dirección, rol y límite de préstamos | `users` con la estructura de autenticación de Laravel, más tipo y número de identificación | Se reutilizó la autenticación que trae Laravel; los campos de préstamo (rol, límite) se agregarán con el módulo de préstamos |
| Disponibilidad | `stock` en `Libro` y `estado_ejemplar` por copia | Solo `libros.stock` | Sin la tabla de ejemplares, el stock se lleva como cantidad; después podrá calcularse contando ejemplares disponibles |
| Autor, Ejemplar, Ubicación, Préstamo | Tablas completas | Sin migrar (datos CSV simulados) | Se priorizó el catálogo y el CRUD de la Clase 5 |
| Reservas | No aparecen en el MER | No existen | La ficha las pide y falta diseñarlas |

## 6. Requerimientos del sistema

La ficha establece, como primer objetivo específico, especificar los requerimientos funcionales y no funcionales. Estos son los que se desprenden de la ficha y del diseño del MER, con su estado en este corte.
...
|---|---|
| Usabilidad | Interfaz clara y con la identidad visual de la institución; nivel de satisfacción de los usuarios de 4/5 o más en la escala Likert |
...
- PHP 8.3 o superior
- Composer
- Docker Desktop (con WSL2 si se trabaja en Windows; conviene correr todo desde el sistema de archivos de WSL y no desde `/mnt/c/...`, porque se vuelve lento)
- Git
...
# Capítulo 4: Estructura del Proyecto, Modelos, Autenticación y Seeders.
...
Además de los requisitos del Capítulo 2, en esta versión se necesita:
...
Esta versión del proyecto también puede ejecutarse directamente con `php artisan serve`, sin contenedores. Hay que tener una base de datos local disponible y ajustar en el `.env` el valor `DB_HOST=127.0.0.1` (en Sail vale `mysql`, que es el nombre del contenedor):
...
| Tema de la Clase 5 | Cómo quedó en el proyecto | Dónde está |
|---|---|---|
| Controlador Resource | `LibroController` con las 7 acciones (`index`, `create`, `store`, `show`, `edit`, `update`, `destroy`) | `app/Http/Controllers/LibroController.php` |
| Rutas con `Route::resource` | Una sola línea genera las 7 rutas del módulo | `routes/web.php` |
| Vistas Blade | Listado, formulario de creación y formulario de edición | `resources/views/libros/` |
| Listado con `with()` (Eager Loading) | El `index` carga editorial y género junto con los libros | `LibroController@index` |
| Scopes | `scopeBuscar()` y `scopeConStock()` en el modelo | `app/Models/Libro.php` |
...
## 5.3 Controlador y eager loading
...
## 5.6 Validación de los formularios
...
## 5.7 Cómo se comprueba que las relaciones funcionan
...
## 5.8 Comandos que usamos seguido
...
# Levantar y apagar el entorno
./vendor/bin/sail up -d
./vendor/bin/sail down
```

## 5.9 Cumplimiento del entregable del primer corte

Revisión punto por punto de lo que exige el entregable (sección 2.2 del documento del primer corte):

| Requisito | Estado | Evidencia |
|---|---|---|
| Proyecto Laravel con Sail configurado | Hecho | `compose.yaml`, Capítulo 2 |
| Migraciones para mínimo 3 tablas relacionadas | Hecho | `editorials`, `generos` y `libros` (más `users`), en `database/migrations/` |
| Modelos con `$fillable` y relaciones | Hecho | `app/Models/`, Capítulo 4 |
| Seeders con mínimo 10 registros por tabla | Verificar | `database/seeders/`. Confirmar el conteo en cada tabla; `users` solo tiene el usuario de prueba |
| `index.blade.php` de la entidad principal | Hecho | `resources/views/libros/` |
| `create.blade.php` con validación básica | Verificar | Reglas en la sección 5.6; confirmar que coincidan con el código y que los mensajes de error se muestren en la vista |
| `edit.blade.php` | Hecho | `resources/views/libros/` |
| Controlador Resource y `Route::resource` | Hecho | Secciones 5.2 y 5.3 |
| Uso de `with()` para evitar el problema N+1 | Hecho | Sección 5.3 |
| README actualizado con instrucciones de instalación | Hecho | Este archivo, Capítulo 2 |
| Análisis del negocio en `docs/analisis.md` | Hecho | Capítulo 1 |
| Diccionario de datos en `docs/diccionario.md` | Hecho | Capítulo 1, sección 4 |
| Diagrama ER en `docs/diagrama_mer.png` | Verificar | El archivo del repositorio es `diagrama_mer.jpeg`; falta exportar también el PNG o mantener el nombre `.jpeg` en todas las referencias |

## 5.10 Dónde está la evidencia de cada criterio de la rúbrica

| Criterio | Peso | Dónde se ve |
|---|---|---|
| Análisis del negocio | 20 % | Capítulo 1 y `docs/analisis.md` |
| Diagrama MER | 15 % | Capítulo 1, sección 5 y `docs/diagrama_mer.jpeg` |
| Migraciones y modelos | 20 % | `database/migrations/` y `app/Models/` |
| Relaciones Eloquent | 15 % | Capítulo 4 y prueba en Tinker (sección 5.7) |
| Vistas funcionales | 15 % | CRUD de libros en `/libros` (sección 5.5) |
| Presentación | 10 % | Sustentación del 24 de septiembre (10 minutos por grupo) |
| Código en GitHub | 5 % | Este repositorio, con README y carpeta `docs/` |

## 5.11 Estado actual y próximos pasos

**Lo que ya funciona:** autenticación con verificación de correo, dashboard, catálogo de libros con CRUD completo, relaciones editorial-libro y género-libro, búsqueda por título o ISBN, filtro por stock, seeders y entorno reproducible con Docker.
...
|---|---|---|
| 1 | Migrar `autor`, `autor_libro`, `ubicacion` y `ejemplar`, con sus modelos, relaciones y seeders | Objetivo 2 (arquitectura de datos) |
| 2 | Pasar el género a la tabla puente `libro_genero` (N:M) y ampliar `users` con rol y límite de préstamos | Objetivos 2 y 3 |
| 3 | Módulo de préstamos: `prestamo` y `prestamo_ejemplar`, con registro de préstamo, devolución y cálculo de multas | Objetivo 3 |
| 4 | Diseñar la tabla de reservas y construir el módulo de reservas remotas | Objetivo 3 |
| 5 | Rol de usuario final y consulta de disponibilidad en tiempo real, sin necesidad de ir a la biblioteca | Objetivo 3 |
| 6 | Reemplazar los datos CSV del dashboard por consultas a la base de datos | Objetivo 3 |
| 7 | Evaluar el prototipo: pre-test y post-test, cuestionario Likert de 19 ítems y entrevistas | Objetivo 4 |
